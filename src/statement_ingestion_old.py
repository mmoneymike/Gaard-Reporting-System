...
def build_statement_sections(path: str | Path) -> StatementSections:
    ...
    # New: Introudction: contains Name and Account 
    # New: Key Statistics: Pull MetaInfo, Analysis Period, "October 1, 2025 - December 31, 2025"
    #       Key Statistics replaces Change in NAV
    # We want to pull these columns from this section: Key Statistics,Header,BeginningNAV,EndingNAV,CumulativeReturn, MTM,Deposits & Withdrawals,Dividends,Interest,Fees & Commissions,Other,ChangeInNAV
    ...
    # Risk Measures: we will be pulling from the since inception files for risk metrics not the quarter
    # Use Performance by Symbol to Pullthe Avg Weight and Return, but only show in the pdf if Open=Yes
    # Need to check if the performance includes dividends
    # However, we will not use Performance by Asset Class, but will use Performance by Symbol to calculate the 
    #   performance by asset class, as our asset classes are custom to the csv

    # Use Allocation by Asset Class (last row in this section) to calculate allocation percentages
...
# New csv format: Pull the analysis period.
# we mainly want the section 'Cumulative Performance Statistics": Cumulative Performance Statistics,MetaInfo,Analysis Period,"July 30, 2025 - February 23, 2026"
# Cumulative Performance Statistics,Header,Date,Account,Return
